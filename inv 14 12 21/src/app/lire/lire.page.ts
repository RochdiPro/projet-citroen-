...
export class LirePage  {
 ...
 mode_unique(e): void {
  ...
  {    
    ...
    this.obj={}
    this.obj.code = this.code_prod_texte;
    this.obj.qte= this.nb.value ;   
    this.vider_produit();
    this.vider_nb();
    this.ajouter(this.obj);
  ...
  ajouter (object: any) {
    ...
    else if(this.test_obj==0)
    {
      ...
      for (var item in this.liste ) {      
        if ((this.liste[item].Code == object.code) == true) {         
             this.liste[item].Quantite = parseInt( parseInt(this.liste[item].Quantite) + (parseInt(object.qte)) +"" ) 
             this.openToast();
        }
      }    
    } else{
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: (data: any) => {
            for (var item in this.liste ) {      
              if ((this.liste[item].Code == object.code) == true) {         
                   this.liste[item].Quantite =  (parseInt(object.qte)) ;
                   this.liste[item].modifier =  "oui" ;
                   this.qte_article=(parseInt(object.qte))+"" ;
              }
            }    
           }
        },
          ...
          handler: (data: any) => {
            for (var item in this.liste ) {      
              if ((this.liste[item].Code == object.code) == true) {         
                   this.liste[item].Quantite = parseInt( parseInt(this.liste[item].Quantite) + (parseInt(object.qte)) +"" ) ;
                   this.liste[item].modifier =  "oui" ;
                   this.qte_article=this.liste[item].Quantite+"";
